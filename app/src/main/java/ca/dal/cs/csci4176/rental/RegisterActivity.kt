...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun signUp() {
        ...
        if (name.isBlank()){
            signUpName.error = "Full name is required!"
            signUpName.requestFocus()
            return
        }

        if (email.isBlank()){
            signUpEmailAddress.error = "Email is required!"
            signUpEmailAddress.requestFocus()
            return
        }

        if(!Patterns.EMAIL_ADDRESS.matcher(email).matches()){
            signUpEmailAddress.error = "Please enter a valid email!"
            signUpEmailAddress.requestFocus()
            return
        }
        ...
        if (!isValidPassword(pass)){
            signUpPassword.error = "Password is invalid! (MUST contain at least 8 characters, at least one uppercase letter, at least one lowercase letter, at least one number, at least one special character)"
            signUpPassword.requestFocus()
            return
        }
        ...
        if (pass != confirmPassword) {
            signUpConfirmPassword.error = "Password and Confirm Password do not match"
            signUpConfirmPassword.requestFocus()
            return
        }
